...
- Worn trinkets whose cooldown returns to 30 sec while another trinket is queued will cancel the queued trinket, unless it's a priority trinket of higher ranking. (ie, Earthstrike used in combat, Darkmoon Card queued to swap after combat, 1.5 minutes later if still in combat it will cancel Darkmoon Card swap.)
- "Toggle TrinketMenu" key binding behaves the same as left-click of minimap button: Toggle trinket frame or options if 'Disable Toggle' checked.
- If you use the default UI's Floating Combat Text (and not SCT) on 1.12 client, it will use the default UI's floating text for notifications.
...
*** NOTE ***          TRINKETMENU IS INTENDED TO BE SMALL.        *** NOTE ***
______________________________________________________________________________

TrinketMenu.SetQueue is not meant to be a cure-all or even a widely used feature of this mod.  If you want a mod to script trinket swaps and queues depending on in-game events then you're asking for ItemRack.

If you don't want to use ItemRack (which is fine) please don't expect me to write two versions of ItemRack, one you don't like and one you do like.  Use the gear-swapping mod of your choice.  TrinketMenu will remain feature-creep free.
